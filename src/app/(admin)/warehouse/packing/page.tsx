...
export default async function WarehousePackingPage() {
    ...
    const statusConfig: Record<string, { label: string; color: string; action: string }> = {
        ...
        allocated: { label: 'จัดสรรแล้ว', color: 'bg-teal-100 text-teal-700', action: 'แพ็คสินค้า' },
            ...
            {requests.length === 0 ? (
                ...
                <div className="space-y-3">
                    {requests.map(req => {
                        ...
                        return (
                            <Link key={req.id} href={targetUrl} className="block bg-white rounded-xl p-4 hover:border-teal-300 transition-colors shadow-[0_1px_3px_rgba(0,0,0,0.06)] border border-slate-100">
                                <div className="flex items-center justify-between">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <span className="text-xs text-teal-600 font-medium">{config.action}</span>
