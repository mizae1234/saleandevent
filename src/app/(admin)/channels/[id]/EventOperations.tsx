    ...
    return (
        <div className="rounded-xl bg-white p-6 shadow-[0_2px_10px_-2px_rgba(0,0,0,0.05)] h-full">
            ...
            <div className="space-y-3">
                {/* Pack Goods Button */}
                {isPackable ? (
                    <Link href={`/channels/${channelId}/packing`} className="block">
                        <div className="flex items-center p-3 rounded-lg border border-teal-100 bg-teal-50 hover:bg-teal-100 transition-colors cursor-pointer group">
                            <div className="h-10 w-10 rounded-full bg-teal-200 text-teal-700 flex items-center justify-center mr-3 group-hover:scale-110 transition-transform">
                                <Package className="h-5 w-5" />
                            </div>
                            <div>
                                <h4 className="font-medium text-teal-900">แพคสินค้า</h4>
                                <p className="text-xs text-teal-600">จัดการรายการเบิกและแพคของ</p>
                            </div>
                        </div>
                    </Link>
                ) : status === "packed" ? (
