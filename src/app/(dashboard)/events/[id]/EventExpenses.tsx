...
import { addEventExpense, removeEventExpense } from "@/actions/event-actions";
...
export function EventExpenses({ eventId, expenses }: Props) {
    ...
    const confirmDelete = () => {
        ...
        startTransition(async () => {
            try {
                await removeEventExpense(deleteId, eventId);
                setDeleteId(null);
            } catch (error) {
                console.error(error);
